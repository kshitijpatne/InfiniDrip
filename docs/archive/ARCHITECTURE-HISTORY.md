...
This file preserves the complete pre-Slice-183 contents of `ARCHITECTURE.md`.
It is retained for historical evidence and source context, not as current
architecture, planning, or status instructions. Read the current guide in
`ARCHITECTURE.md`; use `PROJECT-STATE.md` for current status and
`docs/PROJECT-DECISIONS.md` for maintainer decisions.
