...
describe("surfacePlaceable", () => {
  ...
  it("rejects non-objects and broken shapes", () => {
    ...
    expect(surfacePlaceable({ ...placement(), id: "" })).toBe(false);
    expect(surfacePlaceable({ ...placement(), kind: "embroidery" })).toBe(false);
    expect(surfacePlaceable({ ...placement(), zOrder: 0.5 })).toBe(false);
    expect(surfacePlaceable({ ...placement(), sourceName: 7 })).toBe(false);
  });
});
