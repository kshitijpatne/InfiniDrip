...
describe("shared croquis contracts across the garment registry", () => {
  it.each(GARMENT_CASES)("keeps %s on a valid front/side/back contract", (_name, recipe) => {
    const region = recipe.fields.includes("chest") ? "upper" : "lower";
    const front = region === "upper" ? upperBody(recipe, "front") : renderSkirtBody(STANDARD_M);
    const back = region === "upper" ? upperBody(recipe, "back") : renderSkirtBody(STANDARD_M);
    const side = renderSideCroquis(STANDARD_M, region);
